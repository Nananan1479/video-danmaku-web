...
一个前后端分离的类 B 站视频分享与弹幕互动平台。（持续开发中）

该网站已部署在服务器上：**<http://8.138.242.39>**
...
- 打开 `cilicili-back/src/main/resources/example/application-dev.example.yml` 文件
- 找到 `spring.datasource.username` 和 `spring.datasource.password` 行
- 将 `your_db_username` 和 `your_db_password` 替换为你的 MySQL 用户名和密码
- 配置你的Redis（如果有秘密）和 `application.example.yml` 里的阿里云oss
- 将此文件夹里的 `example` 去掉。变为 `application-dev.yml` , `application-prod.yml` , `application.yml`
