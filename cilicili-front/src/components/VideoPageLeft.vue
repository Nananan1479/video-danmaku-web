...
import { sendDanmakuHTTP, fetchDanmakuByVideoId, toggleLike, sendCoin, toggleCollect, getInteractionStatus } from '@/api/index'
...
const interaction = reactive({
    liked: false, coined: false, collected: false,
    likeCount: 0, coinCount: 0, collectCount: 0
})
const isInteractionLoading = reactive({ like: false, coin: false, collect: false })

async function loadInteractionStatus() {
    if (!videoId.value) return
    try {
        const res = await getInteractionStatus(videoId.value)
        if (res?.data?.code === 200 && res.data.data) {
            Object.assign(interaction, res.data.data)
        }
    } catch { /* ignore */ }
}

async function handleToggleLike() {
    if (!videoId.value || isInteractionLoading.like) return
    isInteractionLoading.like = true
    try {
        const res = await toggleLike(videoId.value)
        if (res?.data?.code === 200 && res.data.data) Object.assign(interaction, res.data.data)
    } catch { ElMessage.error('操作失败') }
    isInteractionLoading.like = false
}

async function handleCoin() {
    if (!videoId.value || isInteractionLoading.coin) return
    const num = 1  // 默认投1币，可扩展为选择1或2
    isInteractionLoading.coin = true
    try {
        const res = await sendCoin(videoId.value, num)
        if (res?.data?.code === 200 && res.data.data) Object.assign(interaction, res.data.data)
        else if (res?.data?.message) ElMessage.warning(res.data.message)
    } catch { ElMessage.error('操作失败') }
    isInteractionLoading.coin = false
}

async function handleToggleCollect() {
    if (!videoId.value || isInteractionLoading.collect) return
    isInteractionLoading.collect = true
    try {
        const res = await toggleCollect(videoId.value)
        if (res?.data?.code === 200 && res.data.data) Object.assign(interaction, res.data.data)
    } catch { ElMessage.error('操作失败') }
    isInteractionLoading.collect = false
}
/** 视频数据加载中 */
        ...
        <div class="action-bar">
            <div class="actions">
                ...
                <button class="action-btn icon-videoReward" :class="{ 'action-btn--active': interaction.collected }" @click="handleToggleCollect" :disabled="isInteractionLoading.collect">
                    ...
                    {{ interaction.collectCount || displayCollectCount }}
                </button>
...
.action-btn--active { color: #00AEEC; }
